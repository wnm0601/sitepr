...
// NeuraMotion AI - 2-second opening overlay
// Add this to the bottom of local-ui.js or script.js.

(function initNeuraOpening() {
  const overlay = document.getElementById('nmOpening');
  if (!overlay) return;

  const skipButton = document.getElementById('nmOpeningSkip');
  const storageKey = 'neuraMotionOpeningPlayed';

  const closeOpening = () => {
    overlay.classList.add('is-hidden');
    document.body.classList.remove('nm-opening-active');
  ...
  document.body.classList.add('nm-opening-active');

  const timer = window.setTimeout(closeOpening, 2050);
